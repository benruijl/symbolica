...
impl PackedRationalNumberWriter for Coefficient {
    fn write_packed(&self, dest: &mut Vec<u8>) {
        ...
        match self {
            ...
            Coefficient::FiniteField(num, f) => {
                dest.put_u8(FIN_NUM);
                (*num.get_representation(), f.0 as u64).write_packed(dest); // this adds an extra tag
            }
    ...
    fn write_packed_fixed(&self, mut dest: &mut [u8]) {
        match self {
            ...
            Coefficient::FiniteField(num, f) => {
                dest.put_u8(FIN_NUM);
                (*num.get_representation(), f.0 as u64).write_packed_fixed(dest);
            }
        }
    }
    ...
    fn get_packed_size(&self) -> u64 {
        ...
        match self {
            ...
            Coefficient::FiniteField(m, i) => {
                2 + (*m.get_representation(), i.0 as u64).get_packed_size()
            }
            Coefficient::RationalPolynomial(_) => {
        ...
        if disc == RAT_POLY {
            ...
            (
                CoefficientView::FiniteField(
                    FiniteFieldElement::from_representation(num),
