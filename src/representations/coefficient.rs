...
use crate::{
    coefficient::{BorrowedCoefficient, Coefficient, SerializedRational},
    domains::{
        finite_field::FiniteFieldElement, integer::IntegerRing,
...
impl PackedRationalNumberWriter for Coefficient {
    fn write_packed(&self, dest: &mut Vec<u8>) {
        match self {
            Coefficient::Natural(num, den) => (*num, *den).write_packed(dest),
            Coefficient::Large(r) => {
    ...
    fn write_packed_fixed(&self, mut dest: &mut [u8]) {
        match self {
            Coefficient::Natural(num, den) => (*num, *den).write_packed_fixed(dest),
            Coefficient::Large(_) | Coefficient::RationalPolynomial(_) => {
    ...
    fn get_packed_size(&self) -> u64 {
        match self {
            Coefficient::Natural(num, den) => (*num, *den).get_packed_size(),
            Coefficient::Large(l) => {
...
pub trait PackedRationalNumberReader {
    fn get_number_view(&self) -> (BorrowedCoefficient, &[u8]);
...
impl PackedRationalNumberReader for [u8] {
    ...
    fn get_number_view(&self) -> (BorrowedCoefficient, &[u8]) {
        ...
        if disc == RAT_POLY {
            ...
            (
                BorrowedCoefficient::RationalPolynomial(rat),
            ...
            (
                BorrowedCoefficient::Large(SerializedRational {
                    is_negative: num < 0,
            ...
            (
                BorrowedCoefficient::FiniteField(
            ...
            (BorrowedCoefficient::Natural(num, den), source)
        }
    }
