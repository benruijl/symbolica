...
use symbolica::{
    ...
    rings::{
        finite_field::{self, FiniteField, PrimeIteratorU64},
...
fn finite_field_test() {
    ...
    let f = FiniteField::<u64>::new(prime);
    ...
    let a = AtomTree::Mul(vec![
        AtomTree::Num(Number::FiniteField(f.to_element(13), fi)),
        AtomTree::Num(Number::FiniteField(f.to_element(15), fi)),
        AtomTree::Var(x),
    ]);
