...
impl<F: Ring> Vector<F> {
    ...
    pub fn derivative<T>(&self, x: &T) -> Vector<F>
    where
        F: Derivable<T>,
    {
    ...
    pub fn grad<T>(&self, x: &[T]) -> Vector<F>
    where
        F: Derivable<T>,
    {
        if self.len() != x.len() {
    ...
    pub fn curl<T>(&self, x: &[T]) -> Vector<F>
    where
        F: Derivable<T>,
    {
    ...
    pub fn jacobian<T>(&self, x: &[T]) -> Matrix<F>
    where
        F: Derivable<T>,
    {
...
impl<F: Ring> Matrix<F> {
    /// Compute the derivative in the variable `x`.
    pub fn derivative<T>(&self, x: &T) -> Matrix<F>
    where
        F: Derivable<T>,
    {
        self.map(|e| self.field.derivative(e, x), self.field.clone())
        ...
        if opts.mode.is_latex() {
            ...
            for (ri, r) in self.row_iter().enumerate() {
                ...
                for (ci, c) in r.iter().enumerate() {
                    ...
                    if ci + 1 < self.ncols as usize {
                        write!(f, "{}", ", ")?; //.bold().color(Color::BrightMagenta))?; FIXME
                    }
                }
                f.write_char(']')?;
