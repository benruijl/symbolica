...
use crate::{
    representations::{
        number::Number, Add, Atom, AtomView, Fun, Identifier, ListIterator, ListSlice, Mul, Num,
        SliceType, Var,
    },
    state::State,
};
...
pub enum Pattern {
    ...
    FnWildcard(Identifier, Vec<Pattern>),
    Num(Number),
    Pow(Box<(Pattern, Pattern)>),
    Mul(Vec<Pattern>),
    Add(Vec<Pattern>),
}
...
enum PatternIter<'a, 'b, P: Atom> {
    Literal(Option<usize>, Pattern), // TODO: convert pattern to AtomView
        ...
        'next_match: loop {
            ...
            if forward_pass {
                ...
                let it = match &self.pattern[self.iterators.len()] {
                    Pattern::Var(name) => PatternIter::Literal(None, Pattern::Var(*name)),
                    ...
                    Pattern::FnWildcard(_, _) => todo!(),
                    Pattern::Num(n) => PatternIter::Literal(None, Pattern::Num(n.clone())),
                    Pattern::Pow(_) => todo!(),
                    Pattern::Mul(_) => todo!(),
                    Pattern::Add(_) => todo!(),
                };
            ...
            match self.iterators.last_mut().unwrap() {
                PatternIter::Wildcard(w) => {
                    ...
                    // does not work when empty list is valid solution
                ...
                PatternIter::Fn(index, name, args, s) => {
                    ...
                    while ii < self.target.len() {
                        if self.used_flag[ii] {
                            ii += 1;
                            continue;
                        } else if !first_option && self.ordered_gapless {
                            break;
                        }
                ...
                PatternIter::Literal(index, atom) => {
                    ...
                    while ii < self.target.len() {
                        ...
                        if match (&self.target.get(ii), &atom) {
                            (AtomView::Var(name1), Pattern::Var(name2)) => {
                                name1.get_name() == *name2
                            }
                            (AtomView::Num(n1), Pattern::Num(n2)) => {
                                n1.get_number_view() == n2.to_borrowed()
                            }
                            (AtomView::Var(_), _) => false,
                            (_, Pattern::Var(_)) => false,
                            (AtomView::Num(_), _) => false,
                            (_, Pattern::Num(_)) => false,
                            x => todo!("Implement comparison of {:?}", x.1),
                        } {
                            *index = Some(ii);
