...
mod tests {
    ...
    fn test2() {
        ...
        let series = [
            44, 95, 5, 51, 2, 72, 47, 44, 21, 59, 53, 29, 71, 39, 2, 27, 100, 20,
        ];
        let series_map = series
            .iter()
            .map(|x| field.to_element(*x))
            .collect::<Vec<_>>();

        let (res, it) = field.find_linear_recurrence_relation(&series_map);
        println!("{}", it);

        for x in &res {
            println!("R {}", field.printer(&field.neg(&x)));
        }
