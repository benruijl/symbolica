...
use crate::domains::{
    ...
    rational::{Fraction, FractionField, FractionNormalization, Q, RationalField},
...
impl<F: Ring, E: PositiveExponent> MultivariatePolynomial<F, E, LexOrder> {
    ...
    pub fn evaluate<T: FloatLike, M: Fn(&F::Element) -> T>(
        &self,
        map_coeff: M,
        point: &[T],
    ) -> T {
...
impl<R: Ring, E: Exponent> Derivable<Variable> for PolynomialRing<R, E> {
    fn derivative(
        &self,
