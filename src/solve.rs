...
use crate::{
    ...
    domains::rational_polynomial::{RationalPolynomial, RationalPolynomialField},
    domains::{
        InternalOrdering,
...
mod test {
    ...
    use crate::{
        ...
        domains::rational_polynomial::{RationalPolynomial, RationalPolynomialField},
        domains::{float::F64, float::Real, integer::Z, rational::Q},
        parse,
        poly::Variable,
        symbol,
        tensors::matrix::Matrix,
    ...
    fn solve_from_matrix() {
        ...
        let var_map = m[(0, 0)].get_variables().clone();
        let res = res
            .iter()
            .map(|x| parse!(x).to_rational_polynomial(&Z, &Z, var_map.clone()))
            .collect::<Vec<_>>();
